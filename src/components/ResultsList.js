...
const ResultsList = () => {
  ...
  const goToBooster = ({ setName, setCode, cardCount, setBlock, scryfallUri }) =>
    navigation.navigate("Booster", { setName, setCode, cardCount, setBlock, scryfallUri });

  const hitAPI = async function () {
    const result = await axios.get("https://api.scryfall.com/sets");
    const setsData = result.data.data;
    const sR = setsData.filter(
      (set) =>
        set.set_type === "expansion" ||
        set.set_type === "core" ||
        (set.set_type === "masters" &&
          set.digital === false &&
          set.name !== "Universes Within" &&
          set.name !== "Mystery Booster Retail Edition Foils" &&
          set.code !== "tsb"),
    );
  ...
  const SetIcon = styled(Image)`
    height: ${(props) => props.theme.setIconStyle.height};
    width: ${(props) => props.theme.setIconStyle.width};
    margin-right: ${(props) => props.theme.setIconStyle.marginRight};
  `;
  ...
  const renderItem = ({ item }) => {
    ...
    return (
      <Touchables onPress={() => goToBooster({ setName, setCode, cardCount, setBlock, scryfallUri })}>
        <View style={{ flexDirection: "row", alignItems: "center" }}>
          <SetIcon source={{ uri: setImg }} />
          <SetNameText>{setName}</SetNameText>
        </View>
      </Touchables>
    );
  };
